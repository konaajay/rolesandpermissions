...
def test():
    ...
    res_revoked = requests.get(f'{PYTHON_URL}/leads/', headers=userA_headers_revoked)
    ...
    res_restored = requests.get(f'{PYTHON_URL}/leads/', headers=userA_headers_restored)
    ...
    # Use the restored token (Tenant A) but spoof Tenant B code
    res_spoof = requests.get(f'{PYTHON_URL}/leads/', headers={'Authorization': f'Bearer {res_userA_restored.json()["token"]}', 'X-Tenant-Code': tenantB_code})
